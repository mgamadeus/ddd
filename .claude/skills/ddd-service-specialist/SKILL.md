...
Entities are rights-protected at the database query level via three overridable static methods in `DatabaseRepoEntity` (override in `DB{EntityName}` classes):

| Method | Default | Called During |
|--------|---------|--------------|
| `applyReadRightsQuery()` | `false` (no restrictions) | `find()`, `findAll()` |
...
All gated by `$applyRightsRestrictions` (default `true`). When `false`, all rights checks are bypassed (used for internal/system operations).

### leftJoin Alias Convention

When using `leftJoin()` inside rights queries, the join alias **MUST** use `{ModelAlias}_{propertyName}_rights` to prevent alias collision with the expand system:

```php
$alias = static::getBaseModelAlias();

// WRONG -- collides with expand alias convention
...
For chained joins, each alias builds on its parent:
```php
$supportTicketAlias = $alias . '_supportTicket_rights';
$supportContactAlias = $supportTicketAlias . '_supportContact_rights';
```

### Standard Rights Query Skeleton
...
public static function applyReadRightsQuery(DoctrineQueryBuilder &$queryBuilder): bool
{
    ...
    // 3. Admin check: admins pass through
...
| Scenario | Approach |
|----------|----------|
| Entity has `accountId` -- restrict to own | Direct `accountId = :authAccountId` filter |
| Entity has `worldId` -- scope to tenant | Direct `worldId IN (...)` filter |
| Entity relates to entity with filtering column | `leftJoin` with `{ModelAlias}_{property}_rights` alias |
| Entity 2+ relations away from filter target | Multi-level join chain or DQL subquery |
| Write stricter than read | Override `applyUpdateRightsQuery()`, call parent read first |
| Hide fields not filter rows | Override `mapToEntity()` with `addPropertiesToHide()` |

### Subquery Pattern (No leftJoin Needed)

When the entity has no direct relation to the filtering target:
...
Always use `Entity::getRepoClassInstance()` + `::BASE_ORM_MODEL` for model class names in subqueries.

### Custom Update Rights
...
For hiding sensitive properties after loading (not filtering rows):
...
public function mapToEntity(
    ...
    if (self::$applyRightsRestrictions && (!$authAccount || !$authAccount->roles->isAdmin())) {
        $entity->addPropertiesToHide('email', 'password', 'gender', 'age');
    }
    return $entity;
}
```
