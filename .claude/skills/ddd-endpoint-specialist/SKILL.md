...
#[Update]                    // binds PATCH + POST (NOT PUT — the PUT verb is a separate #[Put] attribute)
...
#[Enum('ACTIVE', 'INACTIVE')]  // ⚠ NOT read by the OpenAPI documenter (no reader for #[Enum] in src/) — does not restrict the schema. Use a backed PHP enum type or #[Choice] for an enforced enum.
```
...
> **CRITICAL:** Response DTOs MUST extend `RestResponseDto`. (There is no `ResponseDto` class — `RestResponseDto` is the base.)
...
- **Typed arrays: use the BRACKET form `@var T[]` (`@var string[]`, `@var Foo[]`) — NEVER the generic `@var array<T>`.** The autodocumenter reads an array's element type from the `@var` docblock; `array<T>` mis-generates the schema (`items.type: array` + the `<T>` leaks into the description) → a hard Gemini/Google-AI-Studio **400 INVALID_ARGUMENT** on any MCP-exposed DTO. Maps (`array<K,V>`) aren't supported either — model them as a typed VO/Entity. See "How DTOs & Entities Become the API Schema" below for the full mechanism.
...
| `object` | *intended* rejection, but currently EMITTED as `{type: object}` — a `SchemaProperty` defect (`SchemaProperty.php:219-224`, see note under the attribute table); prefer a typed class or `array` regardless |
...
```php
/** @var string[] Keyword names to save */    // ✅ → items: {type: string},  description: "Keyword names to save"
public array $keywords;

/** @var array<string> Keyword names to save */  // ❌ element type resolves to `array`, and "<string>" LEAKS into
public array $keywords;                           //    the description → items: {type: array} (no leaf type) →
                                                  //    Google AI Studio rejects the tool with 400 INVALID_ARGUMENT
```
...
| `#[Enum('A','B')]` | ⚠ NOT read by the documenter — no effect on the schema (use a backed enum or `#[Choice]`) |
...
- **QueryOptions grammar** (`filters`/`orderBy`/`expand`/`select` syntax) → `QueryOptionsSyntax::getSyntaxDocumentation()` once; each param keeps only its endpoint-specific allowed-property list. See `ddd-query-options-specialist`.
- **Shared request params** (`outputFormat`/`noCache`/`skip`/`top`/`skiptoken`) → mark with `#[SharedRequestParameter]`; the full doc is emitted once via `SharedRequestParametersSyntax`, the per-param description shrinks to a pointer.
- **Narrow which QueryOptions families a DTO advertises** with `#[DtoQueryOptions(expose: [DtoQueryOptions::FILTERS, …])]` — documentation-only (properties stay hydrated/validated/writable). ONLY narrow a family the repo genuinely does not apply: realistic for **Argus-backed** reads, **never** a DB-backed read (Doctrine always applies select/expand/orderBy).
...
- Extend `RestResponseDto` for REST responses (there is no `ResponseDto` class)
