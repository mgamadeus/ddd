...
A list property's PHP type is just `array`; it carries no element type. The documenter recovers it from the **docblock** via `ReflectionArrayType::getArrayType()`, which understands the **bracket form `T[]`** but NOT the generic form `array<T>`:
